...
inductive LocalRule : Finset Formula × Finset Formula → Finset ((Finset Formula) × (Finset Formula)) → Type
  -- bot rules:
  ...
  -- nCo rules:
  | nCoL {X Y ϕ ψ} (h : ~(ϕ⋀ψ) ∈ X) : LocalRule ⟨X, Y⟩ {⟨X \ {~(ϕ⋀ψ)} ∪ {~ϕ}, Y⟩, ⟨X \ {~(ϕ⋀ψ)} ∪ {~ψ}, Y⟩}
  | nCoR {X Y ϕ ψ} (h : ~(ϕ⋀ψ) ∈ Y) : LocalRule ⟨X, Y⟩ {⟨X, Y \ {~(ϕ⋀ψ)} ∪ {~ϕ}⟩, ⟨X, Y \ {~(ϕ⋀ψ)} ∪ {~ψ}⟩}


-- If X1 is not simple, then a local rule can be applied.
-- (page 13)
theorem notSimpleThenLocalRuleL {X1 X2} : ¬Simple X1 → ∃ B, Nonempty (LocalRule ⟨X1, X2⟩ B) :=
  by
  ...
  case neg ψ =>
    ...
    case And ψ1 ψ2 =>
      use {⟨X1 \ {~(ψ1⋀ψ2)} ∪ {~ψ1}, X2⟩, ⟨X1 \ {~(ψ1⋀ψ2)} ∪ {~ψ2}, X2⟩};
      use LocalRule.nCoL ϕ_in_X1
    case box => tauto
  case And ψ1 ψ2 =>
    use{⟨X1 \ {ψ1⋀ψ2} ∪ {ψ1, ψ2}, X2⟩};
    use LocalRule.ConL ϕ_in_X1
  case box => tauto

-- If X2 is not simple, then a local rule can be applied.
-- (page 13)
theorem notSimpleThenLocalRuleR {X1 X2} : ¬Simple X2 → ∃ B, Nonempty (LocalRule ⟨X1, X2⟩ B) :=
  by
  ...
  case neg ψ =>
    ...
    case And ψ1 ψ2 =>
      use {⟨X1, X2 \ {~(ψ1⋀ψ2)} ∪ {~ψ1}⟩, ⟨X1, X2 \ {~(ψ1⋀ψ2)} ∪ {~ψ2}⟩};
      use LocalRule.nCoR ϕ_in_X2
    case box => tauto
  case And ψ1 ψ2 =>
    use {⟨X1, X2 \ {ψ1⋀ψ2} ∪ {ψ1, ψ2}⟩};
...
theorem localRulesDecreaseLengthL (r : LocalRule ⟨X1, X2⟩ B) : ∀ pair ∈ B, lengthOfPair pair < lengthOfPair ⟨X1, X2⟩ :=
  by
  cases r
...
inductive LocalTableau : Finset Formula × Finset Formula → Type
  | byLocalRule {X1 X2 B} (_ : LocalRule ⟨X1, X2⟩ B) (next : ∀ pair ∈ B, LocalTableau pair) : LocalTableau ⟨X1, X2⟩
