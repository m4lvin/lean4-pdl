import Pdl.UniCompleteness.UniBuildTreeModel
import Pdl.Completeness.BuildTreeExistence

/-! # From winning strategies to model graphs, part 3, for the uniform game

This is the analogue of `Pdl.Completeness.BuildTreeExistence` for the uniform tableau game,
continuing `Pdl.Completeness.UniBuildTreeModel`. Here we prove the existence lemmas
6.18, 6.19 and 6.20 that are needed for Theorem 6.21 (`UniGame.strmg`).

Everything lives in the namespace `UniGame`. Statements that do not mention `BuildTree`,
`Match` or `PreState` are not repeated here but reused from
`Pdl.Completeness.BuildTreeExistence`.
-/
...
-- The lemmas `Sequent.not_closed_of_sub`, `PdlRule.exists_freeStep` and
-- `PdlRule.exists_loadStep` do not mention `BuildTree` and are reused from
-- `Pdl.Completeness.BuildTreeExistence`.
