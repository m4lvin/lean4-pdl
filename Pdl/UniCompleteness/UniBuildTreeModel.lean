...
/-! # From winning strategies to model graphs, part 2, for the uniform game

This is the analogue of `Pdl.Completeness.BuildTreeModel` for the uniform tableau game,
continuing `Pdl.Completeness.UniBuildTree`. Here we define the model graph
`UniGame.BuildTree.toModel` obtained from a `UniGame.BuildTree` (Definition 6.17) and provide
the same infrastructure as in the original file.

Everything lives in the namespace `UniGame`. All statements that do not mention `BuildTree`,
`Match` or `PreState` --- for example `PdlRule.exists_modL` and `Sequent.box_mem_LR_of_mem_toFinset`
--- are *not* repeated here but reused from `Pdl.Completeness.BuildTreeModel`.
-/
...
@[simp]
noncomputable def BuildTree.toModel {X} (bt : BuildTree [] X) :
...
The lemmas `PdlRule.exists_modL` and `PdlRule.exists_modR` do not mention `BuildTree`
and are reused from `Pdl.Completeness.BuildTreeModel`. -/
...
-- The lemmas `Sequent.isLoaded_of_negLoad_mem`, `Sequent.box_mem_LR_of_mem_toFinset` and
-- `LoadFormula.box_unload` are reused from `Pdl.Completeness.BuildTreeModel`.
...
-- The lemma `AnyFormula.loadBoxes_ne_normal` is reused from
-- `Pdl.Completeness.BuildTreeModel`.

/-! ### The `Q` relation for pre-states -/

-- The lemma `Sequent.negLoad_mem_wForms_iff` is reused from
-- `Pdl.Completeness.BuildTreeModel`.
