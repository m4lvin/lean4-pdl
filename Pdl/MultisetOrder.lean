...
lemma dmlt_trans {α} [pre : Preorder α] [dec : DecidableEq α]:
  ...
  · simp_all only [Multiset.empty_eq_zero, ne_eq]
    rcases Multiset.exists_mem_of_ne_zero X1_ne with ⟨a, a_in⟩
    simp only [Multiset.eq_zero_iff_forall_not_mem, Multiset.mem_add, not_or, not_forall, not_and, Decidable.not_not]
    use a
    tauto
  · rw [P2_def]
  ...
  · intros y y_in_union
    ...
    · have y_in : y ∈ (Y1 - X2) := by simp_all only [Multiset.mem_add, false_or]
      ...
      have y_in_Y1 : y ∈ Y1 := by
        have : Y1 - X2 ≤ Y1 := by
          simp_all only [Multiset.empty_eq_zero, ne_eq, Multiset.mem_add, or_true,
            tsub_le_iff_right, le_add_iff_nonneg_right]
          exact Multiset.zero_le X2
