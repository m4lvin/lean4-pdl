...
lemma firstBox_isAtomic_of_basic (Y_bas : Y.basic)
  ...
  all_goals
    ...
    simp only [List.append_assoc, List.mem_append, Option.mem_toList, Option.map_eq_some_iff,
      Sum.exists, Sum.elim_inl, negUnload, Sum.elim_inr, Formula.basic, decide_false,
      decide_true] at Y_bas
    have := fun h => Y_bas.1 (~⌈β⌉⌈⌈βs⌉⌉φ) (Or.inr (Or.inr h))
    unfold AnyNegFormula.in_side at anf_in_Y
    simp only [AnyFormula.loadBoxes_cons, Formula.neg.injEq] at *
    subst anf_in_Y
    simp only [AnyFormulaBoxBoxes_eq_FormulaBoxLoadBoxes_inside_unload, Formula.box.injEq,
      Option.some.injEq, Sum.inl.injEq, Sum.inr.injEq, exists_eq_left', exists_false, false_and,
      false_or, or_false, reduceCtorEq, true_and] at *
    specialize this ?_
