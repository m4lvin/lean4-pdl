...
lemma subprograms_voc {α β} : β ∈ subprograms α → β.voc ⊆ α.voc := by
  cases α <;> simp only [subprograms, List.cons_append, List.nil_append, List.mem_cons,
    List.mem_append, Program.voc]
  · simp_all
  case sequence α1 α2  =>
...
theorem testsOfProgram.voc α {τ} (τ_in : τ ∈ testsOfProgram α) : τ.voc ⊆ α.voc := by
  cases α <;> simp_all only [testsOfProgram, List.mem_cons, List.mem_append, List.not_mem_nil,
    or_false, Program.voc, subset_refl]
  case sequence α β =>
    intro x x_in
...
theorem freshVarForm_is_larger (φ) : ∀ n ∈ φ.voc.atomProps, n < freshVarForm φ := by
  cases φ
  any_goals (simp [freshVarForm, Formula.voc, Vocab.atomProps]; done)
  case neg φ =>
    have IH := freshVarForm_is_larger φ
    simp_all [freshVarForm]
  case and φ1 φ2 =>
    have IH1 := freshVarForm_is_larger φ1
    have IH2 := freshVarForm_is_larger φ2
    simp_all only [Formula.voc, freshVarForm, lt_sup_iff, Vocab.atomProps]
    aesop
  case box α φ =>
...
theorem freshVarProg_is_larger (α) : ∀ n ∈ α.voc.atomProps, n < freshVarProg α := by
  cases α
  any_goals (simp [freshVarProg, Program.voc, Vocab.atomProps]; done)
  case union α β =>
    have IHα := freshVarProg_is_larger α
    have IHβ := freshVarProg_is_larger β
    simp_all only [lt_sup_iff, Vocab.atomProps, freshVarProg, Program.voc, Vocab.atomProps]
    aesop
  case sequence α β =>
    have IHα := freshVarProg_is_larger α
    have IHβ := freshVarProg_is_larger β
    simp [Vocab.atomProps, freshVarProg, Program.voc, Vocab.atomProps] at *
    aesop
  case star α =>
    have IHα := freshVarProg_is_larger α
