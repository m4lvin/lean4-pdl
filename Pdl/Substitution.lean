...
theorem repl_in_model_sat_iff x ψ φ {W} (M : KripkeModel W) (w : W) :
  ...
  case atom_prop c =>
    simp [evaluatePoint, vDash.SemImplies, evaluate, repl_in_model]
    aesop
  case neg φ =>
...
theorem repl_in_model_rel_iff x ψ α {W} (M : KripkeModel W) (w v : W) :
  ...
  case test φ =>
    have IHφ := repl_in_model_sat_iff x ψ φ M v
    simp only [repl_in_P, relate, and_congr_right_iff] at *
    intro w_is_v
    subst w_is_v
    exact IHφ
end
...
theorem substitutionLemmaRel (σ : Substitution) α {W} (M : KripkeModel W) (w v : W) :
  ...
  case test φ =>
    have IHφ := substitutionLemma σ φ M v
    simp only [subst_in_P, relate, and_congr_right_iff] at *
    intro w_is_v
    subst w_is_v
    exact IHφ
end
